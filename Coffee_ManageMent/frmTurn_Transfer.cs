...
{
    public partial class frmTurn_Transfer : Form
    {
        ...
        {
            Fill_Store();
            txtReceipt_Number.Text = txtCode.Text = txtTransferee.Text = "";
            txtName.Text = txtNotice.Text = "";
            txtCounter.Text = "0";
            txtDate.Text = clsFunction.M2SH(DateTime.Now);
            string query2 = "SELECT MAX (ID) FROM tbl_Header_TurnTransfer";
        ...
        {
            Form_Load();
            SqlDataAdapter da = new SqlDataAdapter("SELECT Code, Date, Transferee, Receipt_Number, Notice FROM tbl_Header_TurnTransfer WHERE ID= '" + ID + "'", dataconnection);
            DataTable dt = new DataTable();
            da.Fill(dt);
            ...
            txtReceipt_Number.Text = dt.Rows[0].ItemArray[3].ToString();
            txtNotice.Text = dt.Rows[0].ItemArray[4].ToString();
            SqlDataAdapter da2 = new SqlDataAdapter("SELECT tbl_Body_TurnTransfer.P_Code, tbl_Product.P_Show_Name, tbl_Body_TurnTransfer.Entity, tbl_Unit.Unit_Name FROM tbl_Body_TurnTransfer INNER JOIN tbl_Product ON tbl_Body_TurnTransfer.P_Code = tbl_Product.P_Code INNER JOIN tbl_Unit ON tbl_Product.Unit = tbl_Unit.Unit_Code WHERE tbl_Body_TurnTransfer.ID='" + ID + "'", dataconnection);
        ...
        public void Delete_Data(int ID)
        {
            try
            {
                string query1 = "DELETE FROM tbl_Body_TurnTransfer WHERE ID = '" + ID + "'";
                string query2 = "DELETE FROM tbl_Tour_Of_Product WHERE Parent=14 AND Parent_Code = '" + Convert.ToInt32(txtID.Text) + "'";
        ...
        public frmTurn_Transfer()
        {
            InitializeComponent();
        }

        private void frmTurn_Transfer_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if (op == "Add")
                        {
                            query1 = "INSERT INTO tbl_Header_TurnTransfer (ID, Receipt_Number, Code, Date, Transferee, Notice, Store, Type, Validation) VALUES ('" + Convert.ToInt64(txtID.Text) + "','" + txtReceipt_Number.Text + "','" + txtCode.Text + "','" + txtDate.Text + "',N'" + txtTransferee.Text + "',N'" + txtNotice.Text + "','" + Store_Code + "',14,'" + "خیر" + "')";
                            if (clsFunction.Execute(dataconnection, query1) == true)
                            {
                                for (int i = 0; i < dgvStore.Rows.Count; i++)
                                {
                                    try
                                    {
                                        query2 = "INSERT INTO tbl_Tour_Of_Product (P_Code, Date, Destination, Issued, Notice, Parent, Parent_Code, Store) VALUES ('" + Convert.ToInt32(dgvStore.Rows[i].Cells["Code"].Value) + "','" + txtDate.Text + "','" + txtName.Text + "','" + float.Parse(dgvStore.Rows[i].Cells["Entity"].Value.ToString()) + "',N'" + txtNotice.Text + "',14,'" + Convert.ToInt32(txtID.Text) + "','" + Store_Code + "')";
                                        query3 = "INSERT INTO tbl_Body_TurnTransfer (ID, P_Code, Entity) VALUES ('" + Convert.ToInt64(txtID.Text) + "','" + Convert.ToInt32(dgvStore.Rows[i].Cells["Code"].Value) + "','" + float.Parse(dgvStore.Rows[i].Cells["Entity"].Value.ToString()) + "')";
                        ...
                        else if (op == "Edit")
                        {
                            ...
                            query1 = "UPDATE tbl_Header_TurnTransfer SET Receipt_Number = '" + txtReceipt_Number.Text + "', Code = '" + txtCode.Text + "', Date = '" + txtDate.Text + "', Transferee = N'" + txtTransferee.Text + "', Notice = N'" + txtNotice.Text + "', Store = '" + Store_Code + "' WHERE ID = '" + Convert.ToInt64(txtID.Text) + "'";
                            if (clsFunction.Execute(dataconnection, query1) == true)
                            {
                                for (int i = 0; i < dgvStore.Rows.Count; i++)
                                {
        ...
        private void txtReceipt_Number_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyValue == 13)
                cmbStore.Focus();
        }
