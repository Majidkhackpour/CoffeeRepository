...
{
    public partial class frmWeb_Favoriets : Form
    {
        SqlConnection dataconnection = new SqlConnection(clsFunction.Connection_string);
        public string URL;
        public frmWeb_Favoriets()
        {
            InitializeComponent();
        }

        private void frmWeb_Favoriets_Load(object sender, EventArgs e)
        {
            string Query = "SELECT ID, Link FROM tbl_Web_Favorites ORDER BY ID DESC";
        ...
        private void mnuDelete_Click(object sender, EventArgs e)
        {
            try
            {
                string query1 = "DELETE FROM tbl_Web_Favorites WHERE ID='" + Convert.ToInt32(dgvStore.CurrentRow.Cells["ID"].Value.ToString()) + "'";
                ...
                if (f.Resault == 0)
                {
                    if (clsFunction.Execute(dataconnection, query1) == true)
                        frmWeb_Favoriets_Load(null, null);
                }
            }
            catch
            {
        ...
        private void mnuDel_All_Click(object sender, EventArgs e)
        {
            try
            {
                string query1 = "DELETE FROM tbl_Web_Favorites";
                frmMessage f = new frmMessage();
